...
waterbalance.ODE <- function(Pe = Pe, u = u, L = L, int_term = int_term,     
  ...
  ETfun = function(w) {
    Eout_l <- Pfun(w) + Eo - ( (Pfun(w))^omega + Eo^omega)^(1/omega)
    Eout_o <- Eo  # ocean evap (assumed equal to potential E)
    # account for ocean
    Eout <- ifelse(lmask==1, Eout_l, Eout_o)
    Eout
  }
...
# 3: ISOTOPES
isotopes.ODE <- function(Pe=Pe, u=u, L=L, n=n, kp=kp, rh = rh, lmask=lmask, 
                         d.in_LBC=d.in_LBC, d.in_RBC=-1e3, t = t,
                         r.std=r.std, k=k, a_P=a_P, a_E=a_E, P.tot = P.tot, 
                         ET.tot=ET.tot, SW.tot=SW.tot, w.tot=w.tot, diff_from_min12=diff_from_min12){
  ...
  ETfun = function(r){ # from modified from Craig and Gordon 1965
    # ... if over ocean, set evap ratio equal to initial (d.in_LBC)
    ...
    dx_E <- dx_P + capdelt
    # account for ocean
    dx_E <- ifelse(lmask==1, dx_E, d.in_LBC+diff_from_min12)
