...
VTSolve <- function(){
  ...
  if(length(myDI) == n){DI <- myDI} else{DI <- myDI[1]}
  # DI <- myDI[1]
  ...
  outis <- isotopes.ODE(Pe=Pe, u=u, L=L, n=n, kp=kp, rh = rh, lmask=lmask,
  ...
  outclim <- as.data.table(cbind(T_et, rh, PET/outwb$P, LCL)) 
  colnames(outclim) <- c('Temp', 'rh', 'DI', 'LCL')
  
  # bring it all together
  outdf <- as.data.table(cbind(outwb, outis, outclim))
  outdf$x_km <- x / 1e3
