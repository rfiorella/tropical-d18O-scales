...
@testset "Reference comparison (threaded)" begin
    ...
    for case_file in readdir(fixtures_dir; join=true)
        ...
        @testset "$(basename(case_file))" begin
            ...
            # Where Julia is NaN, ref should also be NaN
            ...
            if n_valid_dp > 0
                ...
                println("  threaded deltap_bar max error: $max_err_dp ($n_valid_dp points)")
            end

            if n_valid_tau > 0
                max_err_tau = maximum(abs.(tau_bar[valid_tau] .- ref_tau[valid_tau]))
                @test max_err_tau < 1e-4
                println("  threaded tau_bar max error: $max_err_tau ($n_valid_tau points)")
            end
        end
    end
end
