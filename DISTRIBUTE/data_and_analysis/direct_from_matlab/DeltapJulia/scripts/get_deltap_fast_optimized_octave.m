function [deltap_bar, tau_bar] = get_deltap_fast_optimized_octave(E, P, UQ, VQ, Tcond, ...
    LAT, LON, LAT2, LON2, delta_e, csv_file, Plim, dmax)
% get_deltap_fast_optimized_octave
% Octave-compatible version of get_deltap_fast_optimized.m
% Replaces griddedInterpolant with interp2 calls.
%
...
% csv_file: name of csv file containing 2 columns:
%   (1) condensation temperature (in C)
%   (2) effective fractionation factors (from Siler et al., 2021)
% Plim: 2D precipitation cutoff values
% dmax: scalar maximum distance (km) to integrate along upstream path
% LAT/LON: regular 2D grids (rows = lat, cols = lon)
% LAT2/LON2: output grid points
...
% Expand grid cyclically (along dimension 1 = rows)
...
%---------------------------------------
% Extract 1D coordinate vectors for interp2
% Convention: rows = longitude (dim 1), cols = latitude (dim 2)
% After cyclic extension, LON(:,1) is the extended longitude vector
% and LAT(1,:) is the latitude vector.
%---------------------------------------
lon_vec = LON(:, 1);    % 1D longitude vector (rows, extended)
lat_vec = LAT(1, :);    % 1D latitude vector (cols)

%---------------------------------------
% Helper: bilinear interpolation using interp2
% interp2(x_col, y_row, V, xq, yq) where:
%   x_col = latitude (columns), y_row = longitude (rows)
%---------------------------------------
do_interp = @(F, lat_q, lon_q) interp2(lat_vec, lon_vec, F, lat_q, lon_q, 'linear');
...
%---------------------------------------
P2    = do_interp(P, LAT2, LON2);
Plim2 = do_interp(Plim, LAT2, LON2);
...
for i = 1:A
    for j = 1:B
        ...
        vq0 = do_interp(VQ, lat0, lon0);
        uq0 = do_interp(UQ, lat0, lon0);
        E0(1)       = do_interp(E, lat0, lon0);
        Tcond0(1)   = do_interp(Tcond, lat0, lon0);
        Fmag0(1)    = sqrt(uq0.^2 + vq0.^2);
        P0(1)       = do_interp(P, lat0, lon0);
        delta_e0(1) = do_interp(delta_e, lat0, lon0);
        ...
        while tau(jj) < 10 && jj < Nmax-1
            ...
            vq1 = do_interp(VQ, lat1, lon1);
            uq1 = do_interp(UQ, lat1, lon1);
            E0(jj+1)       = do_interp(E, lat1, lon1);
            Tcond0(jj+1)   = do_interp(Tcond, lat1, lon1);
            Fmag0(jj+1)    = sqrt(uq1.^2 + vq1.^2);
            P0(jj+1)       = do_interp(P, lat1, lon1);
            delta_e0(jj+1) = do_interp(delta_e, lat1, lon1);
