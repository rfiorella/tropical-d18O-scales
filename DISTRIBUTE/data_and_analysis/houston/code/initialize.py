...
init = {
    ...
    "ZonalQFlux": "UQ",                   # [] name of zonal moisture flux field
    "MeridQFlux": "VQ",                   # [] name of meridional moisture flux field
    ...
    "Tau-bar_ComputeLandOnly": True,                       # [True, False] do not compute tau-bar over ocean?
    "Tau-bar_bbox_LatRange": [28, 32],                     # [minlat, maxlat] limits for computing tau
    "Tau-bar_bbox_LonRange": [263, 267],                     # [minlon, maxlon] limits for computing tau
    "Tau-bar_bbox_Resolution": [0.25, 0.25],                 # [ndeg lat, ndeg lon] number of latitude and lnogitude degrees for sampling bbox (will get interpolated to cesm grid, but coarser grid saves compute time here)
    ...
    "Tau-bar_decomp_local_v_regional_UpwindEffect": False,      # [True, False] whether to do local v regional output with local P impacting remote moisture
    ...
    "Tau-bar_solve_fracEffect_dist_streamline": False,    # [True, False] estimate the distance from sink within which half the Dtaubar change occurs when remote evap is allowed to be attenuated by local P change
    ...
    "ELs_initstate_SameYrSlice": False,              # [True, False] when defining the baseline (init) state, do hold yr_slice constant and compare across time (i.e. time=1 mjjas compared to all other mjjas), or do we compare yr_slice within each tstep (i.e. at some tstep, mjjas compared to ndjfm)
